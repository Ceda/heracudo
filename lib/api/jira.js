...
const updateReviewAppsField = async (issueKey, hostnames) => {
...
const createComment = async (hostnames = required('hostnames')) => {
  ...
  if (issueKey) {
    await updateReviewAppsField(issueKey, hostnames);
  } else {
    console.error(`[heracudo] no Jira issue found for branch "${HEROKU_BRANCH}"`); // eslint-disable-line
  }
};
