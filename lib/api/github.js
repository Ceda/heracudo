...
const createPrLinks = async (hostnames = required('hostnames')) => {
  const prBody = (await getPrBody()) || '';
  const newLines = hostnames
    .filter((hostname) => !prBody.includes(`[${hostname}](${hostname})`))
    .map((hostname) => `${HRCD_GITHUB_LINK_MARKER} [${hostname}](${hostname}) ${HRCD_GITHUB_LINK_READY}`);

  if (!newLines.length) return null;

  return setPrBody([...newLines, prBody].join(splitter));
};
