module.exports = async () => {
    ...
    await asyncForEach(hostnames, async (hostname) => {
      try {
        ...
        await cloudflare.createDnsRecord(zone.zoneId, reviewAppSubdomain, cname)
          .catch(async (error) => {
            if (!cloudflare.isExistingRecordError(error)) throw error;

            // Existing record may point to a stale cname (e.g. recreated review app
            // whose old record was never cleaned up) — sync it to the current one
            const { data: { result: dnsRecords } } = await cloudflare
              .findDnsRecords(zone.zoneId, reviewAppHostname);
            const dnsRecord = dnsRecords.find(({ type }) => type === 'CNAME');

            if (dnsRecord && dnsRecord.content !== cname) {
              await cloudflare.updateDnsRecord(zone.zoneId, dnsRecord.id, dnsRecord.name, cname);
            }
          });
  ...
  if (reviewAppHostnames.length > 0) {
    ...
    try {
      await require('./api/jira').createComment(reviewAppHostnames);
    } catch (error) {
